...
This project uses a shell script to assemble HTML pages from shared partials and per-page source files. This eliminates duplicated boilerplate (`<head>` tags, CSS links, script includes) across all 11 pages.
...
The build script concatenates partials around each page's content and writes the final HTML to the project root (where Nginx expects it).
...
│   ├── head-bottom.html        Favicon, Google Fonts, 6 CSS links
...
Add the `<link>` tag to `src/_partials/head-bottom.html`, then run `./build.sh`. All 11 pages will include it.
